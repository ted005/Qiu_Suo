...
//        self.tableView.footer = MJRefreshAutoNormalFooter(refreshingTarget: self, refreshingAction: "loadMoreData")
    ...
    func constructPostItem(json: JSON) -> PostItem {
        ...
        postItem.time = json["created"].stringValue
...
//            dateFormatter.dateStyle = NSDateFormatterStyle.MediumStyle
            ...
            if let timeStr = mainPost?.time {
                cell.time.text = dateFormatter.stringFromDate(NSDate(timeIntervalSince1970: Double(timeStr)!))
            }
            else{
                //防止出现“time”文本
                cell.time.text = ""
            ...
            if let timeStr = replies[indexPath.row].time {
                cell.postTime.text = dateFormatter.stringFromDate(NSDate(timeIntervalSince1970: Double(timeStr)!))
            }
            else{
                //防止出现“time”文本
                cell.postTime.text = ""
